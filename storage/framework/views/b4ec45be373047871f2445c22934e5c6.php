...
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute('Ruang Kelas - ' . $classroom->name)]); ?>
            ...
            <div class="absolute bottom-6 left-6 right-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <div class="text-white">
                    <div class="flex items-center gap-3 mb-1">
                        <a href="<?php echo e(route('siswa.dashboard')); ?>" class="text-white/80 hover:text-white transition-colors bg-black/20 p-1.5 rounded-lg backdrop-blur-sm">
                    ...
                    <p class="text-sm text-gray-200 ml-10">Pengajar: <?php echo e($classroom->teacher->name ?? 'Admin'); ?></p>
                </div>
            </div>
        </div>
    <?php else: ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <?php $__empty_1 = true; $__currentLoopData = $classroom->materials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $materi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="p-5 border border-gray-100 rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow group">
                            ...
                            <button onclick="document.getElementById('modal-materi-<?php echo e($materi->id); ?>').classList.remove('hidden')" class="text-sm font-medium text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
                        ...
                        <div class="space-y-4">
                            <?php $__empty_1 = true; $__currentLoopData = $classroom->assignments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tugas): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <div class="p-5 border border-gray-100 rounded-xl bg-white shadow-sm hover:border-blue-200 transition-colors">
                                    ...
                                    <?php if(method_exists($tugas, 'questionBankReferences') && $tugas->questionBankReferences->isNotEmpty()): ?>
                                        <div class="mb-3 text-xs text-gray-600">
                                            Referensi Bank Soal:
                                            <ul class="list-disc ml-5 mt-1">
                                                <?php $__currentLoopData = $tugas->questionBankReferences; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ref): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <li><?php echo e($ref->question_text); ?></li>
                                    ...
                                    <div class="flex items-center justify-between mb-4">
                                        <div class="flex items-center gap-1 text-xs font-medium <?php echo e($effectivePast ? 'text-red-500' : 'text-orange-500'); ?>">
                                            ...
                                            <?php if($remedial): ?>
                                                <span class="ml-2 inline-block text-[10px] px-2 py-1 rounded bg-yellow-100 text-yellow-800">
                                                    Remedial sampai <?php echo e(\Carbon\Carbon::parse($remedial->deadline)->format('d M Y H:i')); ?>
                        ...
                        <div class="space-y-4">
                            <?php $__empty_1 = true; $__currentLoopData = $classroom->quizzes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kuis): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <div class="p-5 border border-gray-100 rounded-xl bg-white shadow-sm hover:border-purple-200 transition-colors">
                                    ...
                                    <?php if($quizRemedial && $quizRemedial->note): ?>
                                        <div class="mb-3 p-3 bg-yellow-50 text-yellow-800 text-xs border border-yellow-100 rounded-lg">
                                            Catatan remedial: <?php echo e($quizRemedial->note); ?>

                                        </div>
                                    <?php endif; ?>

                                    <?php if(!$canTakeQuiz && $attempt): ?>
                                    ...
                                    <?php else: ?>
                                        ...
                                        <a href="<?php echo e(route('siswa.kuis', $kuis)); ?>" class="block w-full text-center py-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white font-medium rounded-lg text-sm hover:shadow-md transition-shadow">
                                            <?php echo e($attempt ? 'Kerjakan Remedial' : 'Mulai Kerjakan'); ?>
    ...
    <div id="modal-materi-<?php echo e($materi->id); ?>" class="fixed inset-0 z-50 hidden bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
        <div class="bg-white rounded-3xl w-full max-w-2xl p-8 shadow-xl relative my-8">
            <button onclick="document.getElementById('modal-materi-<?php echo e($materi->id); ?>').classList.add('hidden')" class="absolute top-5 right-5 text-gray-400 hover:text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-full p-2 transition-colors">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <div>
                    <h3 class="text-2xl font-bold text-gray-900 leading-tight"><?php echo e($materi->title); ?></h3>
                    <?php if($materi->segment): ?>
                        <span class="inline-block px-2.5 py-1 bg-gray-100 text-gray-600 text-xs font-semibold rounded-lg mt-2"><?php echo e($materi->segment); ?></span>
                    <?php endif; ?>
                </div>
            </div>

            <div class="bg-gray-50 rounded-2xl p-5 mb-6 text-gray-700 text-sm leading-relaxed border border-gray-100">
                <?php echo e($materi->description ?? 'Tidak ada deskripsi.'); ?>

            </div>

            <div class="flex flex-col sm:flex-row gap-3">
                <?php if($materi->file_path): ?>
                <a href="<?php echo e(Storage::url($materi->file_path)); ?>" target="_blank" class="flex-1 bg-indigo-600 text-white rounded-xl py-3 px-4 font-semibold text-center hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                    Unduh Dokumen
                </a>
                <?php endif; ?>
                <?php if($materi->link_url): ?>
                <a href="<?php echo e($materi->link_url); ?>" target="_blank" class="flex-1 bg-blue-50 text-blue-700 border border-blue-200 rounded-xl py-3 px-4 font-semibold text-center hover:bg-blue-100 transition-colors flex items-center justify-center gap-2">
...
<?php endif; ?>
<?php /**PATH /Users/alvaritzymaulidan/Documents/ClassTrackRepo-main/resources/views/kelas/show-siswa.blade.php ENDPATH**/ ?>