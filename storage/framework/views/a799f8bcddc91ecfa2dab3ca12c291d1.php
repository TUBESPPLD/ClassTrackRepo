...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => 'Edit Profil']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => 'Edit Profil']); ?>
            ...
            <form method="POST" action="<?php echo e(route('profile.update')); ?>" enctype="multipart/form-data">
                ...
                <div class="flex flex-col items-center mb-8">
                    <div class="relative group cursor-pointer" onclick="document.getElementById('foto').click()">
                        <?php if($user->foto): ?>
                            <img src="<?php echo e(Storage::url($user->foto)); ?>" alt="Foto Profil" class="w-24 h-24 rounded-full object-cover shadow-sm border-4 border-white group-hover:opacity-75 transition-opacity">
                        <?php else: ?>
                ...
                <div class="space-y-5">
                    <?php if(in_array($user->role, ['siswa', 'guru'])): ?>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">ID <?php echo e(ucfirst($user->role)); ?></label>
                        <input type="text" value="<?php echo e($user->student_code ?? 'Belum ada ID'); ?>" disabled class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-100 text-gray-500 font-mono text-center tracking-widest text-lg cursor-not-allowed">
                        <p class="text-xs text-gray-500 mt-1 text-center">
                            <?php echo e($user->role === 'siswa' ? 'Gunakan ID ini untuk diberikan kepada Wali Murid.' : 'Gunakan ID ini sebagai identitas Anda.'); ?>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="<?php echo e(old('name', $user->name)); ?>" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" required>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Email</label>
                        <input type="email" name="email" value="<?php echo e(old('email', $user->email)); ?>" class="w-full border border-gray-200 rounded-xl px-4 py-3 bg-gray-50 hover:bg-white focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" required>
                    </div>
...
<?php endif; ?>
<?php /**PATH C:\Users\user\ClassTrackRepo\resources\views/profile/edit.blade.php ENDPATH**/ ?>