                ...
                @forelse($questions as $qb)
                    <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <form method="POST" action="{{ route('guru.bank-soal.delete', [$classroom, $qb]) }}" id="form-delete-soal-{{ $qb->id }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="confirmDelete('form-delete-soal-{{ $qb->id }}', 'Hapus soal ini?')" class="text-xs font-semibold text-red-600 hover:underline">Hapus</button>
                            </form>
                        </div>
                    </div>
                @empty
